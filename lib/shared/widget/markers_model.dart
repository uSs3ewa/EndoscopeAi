...
class MarkersModel {
  ...
  const MarkersModel(this.marksLeftPadding, this.marksRightPadding, {
    required this.sliderKey,
    required this.modelVideoPlayer,
  });

  void dragSlider(DragUpdateDetails details){
              final RenderBox slider = sliderKey.currentContext!.findRenderObject() as RenderBox;
              final localOffset = slider.globalToLocal(details.globalPosition);
              final sliderWidth = slider.size.width - marksLeftPadding - marksRightPadding;
              final totalMs = modelVideoPlayer.totalDuration.inMilliseconds.toDouble();
              final mousePosition = localOffset.dx - marksLeftPadding;
              final tappedMs = clampDouble(mousePosition * totalMs / sliderWidth, 0, totalMs);
  ...
  void seekToMarker(TapDownDetails details){
              final RenderBox slider = sliderKey.currentContext!.findRenderObject() as RenderBox;
              final localOffset = slider.globalToLocal(details.globalPosition).dx - marksLeftPadding;
              final sliderWidth = slider.size.width - marksLeftPadding - marksRightPadding;
              final totalMs = modelVideoPlayer.totalDuration.inMilliseconds.toDouble();
